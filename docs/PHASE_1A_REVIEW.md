...
6. **✓ RESOLVED - Re-evaluate ADR-004:** Formally split `families` and `relationships` tables. See `docs/DECISIONS.md` [ADR-004-REMEDIATION] for implementation details. The separation is now enforced at the schema level via migration `V003__split_families_and_relationships.sql`.
