...
pub fn compute_assertion_idempotency_key<T: Serialize>(
	...
	sorted.sort_by(|a, b| {
		let left = (a.citation_id.to_string(), a.note.clone().unwrap_or_default());
		let right = (b.citation_id.to_string(), b.note.clone().unwrap_or_default());
		left.cmp(&right)
	});
...
mod tests {
	...
	fn idempotency_key_independent_of_citation_order() {
		...
		let key_b = compute_assertion_idempotency_key(
			entity_id,
			"birth_date",
			&"1850-05-01",
			&[c2, c1],
	...
	fn idempotency_key_changes_when_value_changes() {
		let entity_id = EntityId::new();
		let key_a = compute_assertion_idempotency_key(entity_id, "name", &"John Doe", &[])
			.expect("hash a");
		let key_b = compute_assertion_idempotency_key(entity_id, "name", &"John Roe", &[])
			.expect("hash b");
